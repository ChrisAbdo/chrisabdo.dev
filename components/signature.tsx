...
function useAnimationCompletion(isAnimating, delay = 0, duration = 0) {
  const [isCompleted, setIsCompleted] = React.useState(false);

  React.useEffect(() => {
    let timer;
    setIsCompleted(false); // Reset to false before starting timer
...
export default function Siganture() {
  ...
  // New state to track if the animation has started
  ...
  // delay and duration according to your longest animation
  const delay = 1;
  const duration = 0.85;

  // Pass hasStarted instead of isInView
  const isCompleted = useAnimationCompletion(hasStarted, delay, duration);
  ...
  const draw = {
    ...
    visible: (i: any) => {
      ...
      return {
        ...
        opacity: 1,
        transition: {
          pathLength: { delay, duration: 0.85, ease: "linear" }, // changed type to "tween", set duration
  ...
  return (
    <>
      {!isCompleted && (
        <motion.div ref={ref} className="mt-12 sm:mt-16 mb-12">
          <motion.svg
            width="56"
            height="34"
            viewBox="0 0 56 34"
      ...
      {isCompleted && <DifferentComponent />}
    </>
  );
}
...
function DifferentComponent() {
  ...
  return (
    <motion.h1
      ...
      variants={fade}
      className="mt-12 sm:mt-16 mb-12 tracking-[-0.05em]"
    >
